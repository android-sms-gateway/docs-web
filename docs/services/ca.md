...
Currently, the CA allows you to issue certificates for the next purposes:

1. **Private Webhook Certificate**: Certificate can be used for receiving webhooks in local network.
2. **Private Server Certificate**: Certificate can be used for setting upo the Private server in local network.
...
If you want to run a Private server within local network, you can use the project's CA to generate a valid certificate.

## How to Use

There are two main ways to use CA:

1. **CLI**: (Recommended) You can use the [smsgate-ca](https://github.com/android-sms-gateway/cli) command line tool to issue certificates for private webhooks or the private server.
2. **API**: You can use the CA's API to issue certificates for private webhooks or the private server.
...
3. Run the `./smsgate-ca <type> <your-ip>` command, where `<type>` is a type of certificate you want to issue (`webhooks` or `private`), `<your-ip>` is the private IP address of your server.
...
4. Make a request to the CA, replacing `<type>` with the type of certificate you want to issue (`webhooks` or `private`):
    ```sh
    jq -n --arg content "$(cat server.csr)" '{content: $content, type: "<type>"}' | \
    curl -X POST \
