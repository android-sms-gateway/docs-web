...
# Prepare Data SMS
url = "https://api.sms-gate.app/v1/messages"
payload = {
    "dataMessage": {
...
# Send it!
response = requests.post(url, auth=HTTPBasicAuth(SMS_API_USERNAME, SMS_API_PASSWORD), json=payload, headers=headers)
...
- `deviceId`: Specific device identifier (optional but recommended)
...
app.post('/sms-webhook', (req, res) => {
  ...
  if (message.command === "START_IRRIGATION") {
    irrigationSystem.activate(message.data.duration);
    console.log("Irrigation started for", message.data.duration, "minutes");
  }
...
2. **[Try Our API](../../integration/api.md)** - Programmatically send and receive SMS
3. **Build Your First Project** - Harness the power of Data SMS in your own application

**Final Thought**: *"In a world obsessed with high-speed internet, Data SMS reminds us that sometimes the simplest solutions are the most revolutionary. Turn basic text messages into data powerhouses today!"*
